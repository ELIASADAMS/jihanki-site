...
const ARCHIVE = {
    videos: [
        { id:"eli-test-video", title:"DOCUMENTATION / JULIA BARANYUK", date:"2026", type:"video", category:"video", author:"julia", file:"media/video/eli.mp4" }
    ],
    photos: [
        { id:"cover1", title:"COVER 01", date:"TEST", type:"image", category:"photo", author:"shared", file:"media/photo/cover1.png" }
    ],
    notes: [
        ...DAILY_NOTES,
    ...
    yokai: [
        { id:"fuji-six-yokai", title:"富士山六体妖怪", date:"2026", type:"text", category:"yokai", author:"eli", project:"fuji-yokai", file:"documents/yokai/fuji-six-yokai.md" }
    ],
    music: [
    ...
    sound: [
        { id:"hellyea", title:"HELLYEA", date:"TEST FIELD RECORDING", type:"audio", category:"sound", author:"shared", file:"media/sound/hellyea.wav" }
    ],
    text: [
    ...
    projects: [
        { id:"fuji-sonicpi", title:"エリ・ミニン / 青木ヶ原", date:"2026", type:"text", category:"project", author:"eli", project:"fuji-sonicpi", file:"documents/projects/fuji-sonicpi.md" }
    ],
    other: []
};
...
/* Make the long catalogue visibly scrollable on the CRT. */
const archiveStyle = document.createElement("style");
archiveStyle.textContent = `
    #screen-content .archive {
        height: 100%;
        max-height: 100%;
        overflow-y: auto;
        overflow-x: hidden;
        ...
        scrollbar-color: rgba(255,255,255,.38) rgba(255,255,255,.04);
        padding-right: 7px;
    }
    ...
    .markdown-rendered strong { font-weight:700; }
    .markdown-rendered em { font-style:italic; }
    .markdown-rendered p { margin:0 0 14px; }
    .markdown-rendered h1,.markdown-rendered h2,.markdown-rendered h3 { margin:18px 0 9px; line-height:1.25; }
    .markdown-rendered ul,.markdown-rendered ol { padding-left:22px; margin:0 0 14px; }
    .markdown-rendered li { margin:4px 0; }
    .markdown-rendered hr { border:0; border-top:1px solid rgba(255,255,255,.12); margin:18px 0; }
    .markdown-image { display:block; max-width:100%; max-height:300px; object-fit:contain; margin:0 0 16px; }
    .markdown-audio { margin:10px 0 18px; }
    .markdown-audio-label { margin-bottom:6px; }
    .markdown-audio audio { width:100%; }
    .markdown-link { color:inherit; text-decoration:underline; }
`;
...
(function installRuntime(){
    ...
    let ctx=null, buffer=null, source=null, gain=null, loading=null;
    let started=false, muted=false, foreground=null, lastSFX=0;
    ...
    async function loadLoop(){
        ...
        loading=fetch(SOUND.loop,{cache:"force-cache"})
    ...
    async function startLoop(){
        ...
        try{
            await c.resume();
            const b=await loadLoop();
            if(started)return;
            gain=c.createGain();
            gain.gain.value=muted?0:.10;
            gain.connect(c.destination);
            source=c.createBufferSource();
            source.buffer=b;
            source.loop=true;
            source.loopStart=0;
            source.loopEnd=b.duration;
            source.connect(gain);
            source.start(0);
            started=true;
        }catch(_){ }
    ...
    function muteAmbient(value){
        muted=value;
        if(gain&&ctx)gain.gain.setTargetAtTime(value?0:.10,ctx.currentTime,.015);
    }
    ...
    function sfx(name){
        ...
        a.volume=name==="coin"?.55:.40;
        a.play().catch(()=>{});
    }

    function stopForeground(){
        if(foreground){try{foreground.pause();}catch(_){} foreground=null;}
        muteAmbient(false);
    }
    ...
    function inlineMD(text,base){
        let s=escapeHTML(text), tokens=[];
        const token=x=>{const id=`@@JHIKI${tokens.length}@@`;tokens.push(x);return id;};
        ...
        s=s.replace(/\[([^\]]+)\]\(([^)]+)\)/g,(_,label,p)=>{
            const href=safeURL(p,base), lower=p.split("?")[0].toLowerCase();
            if(/\.(mp3|wav|ogg|m4a|aac|flac)$/i.test(lower))return token(`<div class="markdown-audio"><div class="markdown-audio-label">${label}</div><audio controls preload="metadata" src="${escapeHTML(href)}"></audio></div>`);
        ...
        s=s.replace(/\*([^*\n]+?)\*/g,"<em>$1</em>");
        s=s.replace(/_([^_\n]+?)_/g,"<em>$1</em>");
        return s.replace(/@@JHIKI(\d+)@@/g,(_,i)=>tokens[Number(i)]);
    }

    function markdownToHTML(markdown,base){
        const lines=String(markdown).replace(/\r\n?/g,"\n").split("\n");
        let html="", paragraph=[], list=null;
        const flush=()=>{if(paragraph.length){html+=`<p>${inlineMD(paragraph.join(" "),base)}</p>`;paragraph=[];}};
    ...
    async function openMarkdown(item,body){
        try{
            ...
            const doc=new URL(item.file,location.href), base=new URL("./",doc).href;
            body.innerHTML=`<div class="artifact-text markdown-rendered">${markdownToHTML(md,base)}</div>`;
            bindAudio();
        }catch(e){
            body.innerHTML=`<div class="artifact-text"><strong>MEMORY COULD NOT BE READ.</strong><br><br>${escapeHTML(item.file)}<br>${escapeHTML(e.message)}</div>`;
        }
    }
    ...
    window.openItem=async function(item){
        ...
        if(type==="audio"||type==="music"||type==="sound"||/^(mp3|wav|ogg|m4a|aac|flac)$/.test(ext)){
            body.innerHTML=`<audio class="artifact-audio" controls autoplay playsinline src="${escapeHTML(file)}"></audio>`;
            foreground=body.querySelector("audio");muteAmbient(true);
            foreground.addEventListener("play",()=>muteAmbient(true));foreground.addEventListener("pause",()=>muteAmbient(false));foreground.addEventListener("ended",()=>{foreground=null;muteAmbient(false);});return;
        }
        if(type==="video"||/^(mp4|webm|mov)$/.test(ext)){
            ...
            foreground=body.querySelector("video");muteAmbient(true);
            foreground.addEventListener("play",()=>muteAmbient(true));foreground.addEventListener("pause",()=>muteAmbient(false));foreground.addEventListener("ended",()=>{foreground=null;muteAmbient(false);});return;
        }
    ...
    document.addEventListener("pointerdown",e=>{
        ...
        if(!b)return;
        if(b.id==="coin-button")sfx("coin");else if(b.id==="home-button")sfx("home");else sfx("button");
    },{passive:true});

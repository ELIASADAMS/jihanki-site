...
videos: [
  ...
  text: [
    ...
    },
    {
      ...
      author: "eli",
      project: "aokigahara",
      projectPath: "projects/aokigahara/index.html",
      file: "documents/projects/aokigahara.md",
    },
  ],
  projects: [
    {
...
(function installRuntime() {
  ...
  function muteAmbient(value) {
    muted = value;
    if (gain && ctx)
      gain.gain.setTargetAtTime(value ? 0 : 0.1, ctx.currentTime, 0.015);
  }
  function sfx(name) {
    const now = Date.now();
  ...
  function inlineMD(text, base) {
    ...
    s = s.replace(/\*([^*\n]+?)\*/g, "<em>$1</em>");
    s = s.replace(/_([^_\n]+?)_/g, "<em>$1</em>");
    return s.replace(/@@JIHANKI(\d+)@@/g, (_, i) => tokens[Number(i)]);
  }
